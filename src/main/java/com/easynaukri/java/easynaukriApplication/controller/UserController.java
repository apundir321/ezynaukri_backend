...
import com.easynaukri.java.easynaukriApplication.security.IUserService;
import com.easynaukri.java.easynaukriApplication.security.JwtUserDetailsService;
import com.easynaukri.java.easynaukriApplication.security.jwt.AccountCredentials;
import com.easynaukri.java.easynaukriApplication.security.jwt.JwtTokenUtil;
...
public class UserController {
	...
	@Autowired
	private UserProfileRepositoryImpl userProfileRepo;
	...
	public ResponseEntity<?> saveProfile(@RequestParam String userId,@RequestBody RecruiterProfile userProfile,@RequestParam(required = false) String categoryId)
		...
		try {
			profile = userProfileService.updateRecruiterProfile(userId, userProfile,categoryId);
		} catch (Exception e) {
			return new ResponseEntity<GenericResponse>(new GenericResponse("Exception in saving recruiter profile="+e.getMessage()),HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	public List<UserProfile> getProfilesByLocation()
	{
...
//	                .collect(Collectors.toList());
	...
	public ResponseEntity<?> addEducation(@RequestParam String userId,@RequestBody Education education)
			throws Exception {
		UserProfile profile = null;
		try {
			profile = userProfileService.addEducation(education, userId);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/getRecruiterProfile", method = RequestMethod.GET)
	public ResponseEntity<?> getRecruiterProfile(@RequestParam String recruiterId)
			throws Exception {
		User user = null;
		try {
			user = userService.getUserByID(Long.parseLong(recruiterId)).get();
			if(user==null)
			{
	...
	public ResponseEntity<?> filterRecruiterProfiles(@RequestBody String job) {
		...
		try {
			...
			recruiterProfiles = userService.getUserByRoleParams(searchCriteria);
		} catch (Exception e) {
		...
		return new ResponseEntity<List<RecruiterProfile>>(recruiterProfiles, HttpStatus.OK);
	...
	public ResponseEntity<?> addWorkExperience(@RequestParam String userId,@RequestBody WorkExperience workExperience)
			throws Exception {
		UserProfile profile = null;
		try {
			profile = userProfileService.addWorkExperience(workExperience, userId);
		} catch (Exception e) {
			return new ResponseEntity<GenericResponse>(new GenericResponse("Exception in adding education="+e.getMessage()),HttpStatus.INTERNAL_SERVER_ERROR);
		}
		return new ResponseEntity<UserProfile>(profile,HttpStatus.CREATED);
	...
	public ResponseEntity<?> updateTags(@RequestParam String userId,@RequestBody List<String> tags)
			throws Exception {
		UserProfile profile = null;
		...
		try {
			...
			profile = userProfileService.updateTags(skills, userId);
		} catch (Exception e) {
			return new ResponseEntity<GenericResponse>(new GenericResponse("Exception in adding education="+e.getMessage()),HttpStatus.INTERNAL_SERVER_ERROR);
		}
		return new ResponseEntity<UserProfile>(profile,HttpStatus.CREATED);
